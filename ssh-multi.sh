...
# redwhitemiko: https://github.com/redwhitemiko/ssh-multi
...
starttmux() {
    ...
    tmux new-window -n "${windowname}" "ssh -i ~/keys/$key -o StrictHostKeyChecking=no $user@${hosts[0]}"
    unset hosts[0];
    for i in "${hosts[@]}"
    do
        tmux split-window -t "${windowname}" -h "ssh -i ~/keys/$key -o StrictHostKeyChecking=no $user@$i"
